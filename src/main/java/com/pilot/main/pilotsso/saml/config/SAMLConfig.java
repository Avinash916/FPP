...
import com.pilot.main.pilotsso.saml.certificate.KeystoreFactory;
import com.pilot.main.pilotsso.saml.spring.SpringResourceWrapperOpenSAMLResource;
import com.pilot.main.pilotsso.saml.spring.security.SAMLUserDetailsServiceImpl;
...
public class SAMLConfig {
	...
	BeanFactoryPostProcessor idpMetadataLoader() {
		return beanFactory -> {
			...
			try {
				...
				Resource[] idpMetadataFiles = metadataFilesResolver.getResources("classpath:/FederationMetadata.xml");
				...
				Stream.of(idpMetadataFiles).forEach(idpMetadataFile -> {
					try {
						...
						provider.setMetadataTrustCheck(false); // Need to be enabled later
						provider.setMetadataRequireSignature(false);
						String idpName = "Pilot";
    ...
    public MetadataGenerator metadataGenerator(KeyManager keyManager) {
        ...
        generator.setEntityId("https://fuel-pricing-platform-dev.herokuapp.com");
